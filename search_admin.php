<?php
    ...
    WHERE u_user LIKE '%$search_admin%'
    OR u_email LIKE '%$search_admin%'
...
<table class="table table-hover">
    ...
    <?php foreach($result as $row) { ?>
        <tr align="center">
            ...
            <td><a href="update_admin.php?u_id=<?php echo $row['u_id'];?>" data-bs-toggle="modal" data-bs-target="#update<?php echo $row['u_id'];?>"><img src="img/logo/edit.png" width="18"></a></td>
            <td><a href="delete_admin.php?u_id=<?php echo $row['u_id'];?>" data-bs-toggle="modal" data-bs-target="#delete<?php echo $row['u_id'];?>"><img src="img/logo/delete.png" width="20"></a></td>
        </tr>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="update<?php echo $row['u_id'];?>">Edit Admin</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form class="form-horizontal was-validated" name="update" id="update" method="post" action="update_admin.php">
    ...
    <!------------------------------------------------------------------ DELETE ------------------------------------------------------------------>
        ...
        <div class="modal fade" id="delete<?php echo $row['u_id'];?>" tabindex="-1" aria-labelledby="delete<?php echo $row['u_id'];?>" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="delete<?php echo $row['u_id'];?>">Delete Admin</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        Confirm to delete Admin <b><?php echo $row['u_user'];?></b> ?
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <a href="delete_admin.php?u_id=<?php echo $row['u_id'];?>" class="btn btn-danger">Delete</a>
                    </div>
                </div>
            </div>
        </div>
